...
def create_tree_materials(cone_name, leaf_names):
    ...
    cmds.setAttr(f"{tree_material}.baseColor", 0.4, 0.2, 0.05, type="double3")
    cmds.setAttr(f"{tree_material}.specular", 0.1)
    cmds.setAttr(f"{tree_material}.roughness", 0.7)
    ...
    cmds.sets(edit=True, forceElement=tree_sg)
    ...
    cmds.setAttr(f"{leaf_material}.baseColor", 0.1, 0.6, 0.1, type="double3")
    cmds.setAttr(f"{leaf_material}.specular", 0.2)
    cmds.setAttr(f"{leaf_material}.roughness", 0.5)
    cmds.setAttr(f"{leaf_material}.transmission", 0.4)
    cmds.setAttr(f"{leaf_material}.thinWalled", 1)
    ...
    for leaf in leaf_names:
        cmds.select(leaf)
        cmds.sets(edit=True, forceElement=leaf_sg)

    print("✅ متریال‌ها با موفقیت اعمال شدند.")
