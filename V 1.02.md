...
def create_combined_mountain(position=(0, 0, 0), scale=5.0, complexity="simple"):
    # تنظیم فاصله نزدیکتر بین کوه‌ها
    mountain_1_position = [position[0] - scale * 0.5, position[1], position[2]]
    mountain_2_position = [position[0] + scale * 0.5, position[1], position[2]]

    # ساخت دو کوه با پیچیدگی دلخواه
    mountain_1 = create_mountain(position=mountain_1_position, scale=scale, complexity=complexity)
    mountain_2 = create_mountain(position=mountain_2_position, scale=scale, complexity=complexity)

    # ترکیب دو کوه
    combined_mountain_group = cmds.group([mountain_1, mountain_2], name="combined_mountain_group")
    
    return combined_mountain_group
...
def create_ui():
    ...
    cmds.button(label="Create Combined Mountain", command=lambda x: create_combined_mountain(
    position=(0, 0, 0),
    scale=5.0,
    complexity="simple" if cmds.radioButtonGrp("mountainComplexity", query=True, select=True) == 1 else "complex"
))
