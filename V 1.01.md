...
def create_ui():
    ...
    # اضافه کردن دکمه ساخت کوه
    # انتخاب پیچیدگی کوه
    cmds.text(label="Choose Mountain Complexity")
    cmds.optionMenu("mountainComplexity", label="Mountain Complexity")
    cmds.menuItem(label="Simple")  # ساده
    cmds.menuItem(label="Complex") # پیچیده

    # دکمه ساخت کوه
    cmds.button(label="Create Mountain", command=lambda x: create_mountain(
        position=(0, 0, 0),
        scale=5.0,
        complexity=cmds.optionMenu("mountainComplexity", query=True, value=True).lower()
    ))
